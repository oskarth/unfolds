               ...
               {:edit-key :item/text
                ...
                (fn [{:keys [value edit-key]}]
                  (if (:db/id value)
                    ...
                    (put! sync
                       {:op :create :data value})))}}))
          #_(contact-numbers (:person/telephone contact))
          #_(contact-addresses (:person/address contact)))))))
              ...
              (condp = op
                ...
                (let [data (<! (util/edn-chan
                                {:method :post :url "/items"
                                 :data data}))]
                  (.setToken history (str "/" (:db/id data))) ;; XXX
                  (om/transact! app :current-item #(merge % data)))

                :update
                (<! (util/edn-chan
                     {:method :put url (str "/items/" (:db/id data))
...
(util/edn-xhr
 {:method :get
  :url "/items"
  ;; :url "/item/ea72343d-89dc-4dfc-85af-25e1113b0948"
  :on-complete
  (fn [res]
    (prn "NEW ITEMS :" res)
    (swap! app-state assoc :items res)
